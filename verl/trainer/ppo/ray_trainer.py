            ...
            for batch_dict in dataloader_iter:
                ...
                if self.config.trainer.stage == "consolidate":
                    ...
                    if do_profile:
                        ...
                        if self.exp_learner_wg is not None:
                    ...
                    privileged_mode = self.config.trainer.get("privileged_mode", None)
                    use_crop_privileged = privileged_mode == "crop"

                    if use_crop_privileged:
                        EXPERIENCES = [""]
                        EXPERIENCE = ""
                        self.experience = ""
                        rng = None
                        ...
                        with marked_timer("gen", timing_raw, color="red"):
                            if not self.is_textgame:
                                ...
                                for i in range(len(gen_batch_with_exp)):
                                    if use_crop_privileged:
                                        crop_image = gen_batch_with_exp.non_tensor_batch["crop_images"][i][0]
                                        msgs = build_crop_messages_from_raw_prompt(
                                            gen_batch_with_exp.non_tensor_batch["raw_prompt"][i],
